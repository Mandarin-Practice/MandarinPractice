...
export default function Navbar({ theme, toggleTheme }: NavbarProps) {
  ...
  return (
    <header className="bg-accent border-b border-border shadow-md sticky top-0 z-10 opaque-header">
      <div className="container mx-auto px-4 py-4 max-w-5xl">
        ...
        {/* Mobile navigation */}
        {mobileMenuOpen && (
          <nav className="md:hidden mt-4 bg-accent border-t border-border pt-2 opaque-header">
            {navItems.map((item) => (
              <Link 
                key={item.path} 
                href={item.path}
