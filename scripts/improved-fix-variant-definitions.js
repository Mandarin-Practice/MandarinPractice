// Improved script to fix "variant of" definitions with actual meanings
import pkg from 'pg';
const { Pool } = pkg;
...
function extractReferencedCharacter(definition) {
  // Handle both "variant of X" and "old variant of X" patterns
  const match = definition.match(/(?:old )?variant of ([^\[]+)/);
  if (match && match[1]) {
...
async function fixVariantDefinitions() {
  try {
    ...
    // Get all definitions with the "variant of" pattern (including "old variant of")
    ...
    for (let i = 0; i < variantDefs.length; i += BATCH_SIZE) {
      ...
      // Process each definition in parallel for speed
        ...
        try {
          ...
          if (referencedChar) {
            ...
            if (referencedDef) {
              ...
              const newDefinition = `${referencedDef} (variant form)`;
              ...
              if (generatedDef) {
                ...
                // If all else fails, just provide a better fallback than "variant of"
                const fallbackDef = `Alternative form of ${referencedChar}`;
                ...
                console.log(`Used fallback definition for ${def.character}: "${fallbackDef}"`);
                stats.definitionsUpdated++;
              }
            }
          } else {
...
// Main execution
async function main() {
  console.log('Starting improved variant definition fixes...');
